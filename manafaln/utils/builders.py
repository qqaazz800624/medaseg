...
def get_default_path(component_type: Union[ComponentType, str]) -> str:
    if isinstance(component_type, ComponentType):
        mapping = {
            ...
            ComponentType.LOGGER:     ComponentPaths.DEFAULT_LOGGER_PATH,
        }
    else:
        ...
        mapping = {
            ...
            "logger":     ComponentPaths.DEFAULT_LOGGER_PATH,
        }
...
def build_logger(config: Dict) -> Logger:
    ...
    if not isinstance(logger, Logger):
        raise TypeError(f"{type(logger)} is not a valid Logger type")
    return logger
